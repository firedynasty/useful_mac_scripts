-- Transcribe Korean Audio to Text
-- Right-click Quick Action: "workflow receives audio files in any application"
-- Uses whisper-cpp to transcribe Korean, saves .txt next to the original file
...
on run {input, parameters}
	try
		...
		repeat with audioFile in input
			...
			display notification "Transcribing (한국어): " & fileName with title "Whisper Transcribe"

			-- whisper-cli outputs to <input>.txt with -otxt flag
			do shell script "/opt/homebrew/bin/whisper-cli -m " & quoted form of whisperModel & " -f " & quoted form of filePath & " --no-timestamps -otxt -l ko 2>/dev/null"
