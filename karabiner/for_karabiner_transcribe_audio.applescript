...
-- Uses whisper-cpp to transcribe, saves .txt next to the original file
-- Supports: mp3, wav, flac, ogg

on run {input, parameters}
	try
		set whisperModel to "/opt/homebrew/share/whisper-cpp/ggml-base.bin"

		repeat with audioFile in input
			set filePath to POSIX path of audioFile
			set fileName to do shell script "basename " & quoted form of filePath
			display notification "Transcribing: " & fileName with title "Whisper Transcribe"

			-- whisper-cli outputs to <input>.txt with -otxt flag
			do shell script "/opt/homebrew/bin/whisper-cli -m " & quoted form of whisperModel & " -f " & quoted form of filePath & " --no-timestamps -otxt -l auto 2>/dev/null"
