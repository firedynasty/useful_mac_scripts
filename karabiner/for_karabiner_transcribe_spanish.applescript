...
-- Uses whisper-cpp to transcribe Spanish, saves .txt next to the original file
...
on run {input, parameters}
	try
		...
		repeat with audioFile in input
			...
			display notification "Transcribing (Español): " & fileName with title "Whisper Transcribe"

			-- whisper-cli outputs to <input>.txt with -otxt flag
			do shell script "/opt/homebrew/bin/whisper-cli -m " & quoted form of whisperModel & " -f " & quoted form of filePath & " --no-timestamps -otxt -l es 2>/dev/null"
