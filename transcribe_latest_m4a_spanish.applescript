-- Transcribe Latest .m4a from Desktop (Spanish)
-- Save as Application in Script Editor, then add to Dock
-- Finds latest .m4a on Desktop → converts → transcribes Spanish with whisper-cpp → copies text to clipboard
...
on run
	try
		...
		display notification "Converting & transcribing (Español): " & fileName with title "Transcribe M4A"
		...
		-- Transcribe with whisper-cpp (Spanish)
		set transcription to do shell script "/opt/homebrew/bin/whisper-cli -m " & quoted form of whisperModel & " -f " & quoted form of tempWav & " --no-timestamps -l es 2>/dev/null"
