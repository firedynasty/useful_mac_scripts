-- Transcribe Latest .m4a from Desktop (Mandarin)
-- Save as Application in Script Editor, then add to Dock
-- Finds latest .m4a on Desktop → converts → transcribes Mandarin with whisper-cpp → copies text to clipboard
...
on run
	try
		...
		display notification "Converting & transcribing (普通話): " & fileName with title "Transcribe M4A"
		...
		-- Transcribe with whisper-cpp (Mandarin)
		set transcription to do shell script "/opt/homebrew/bin/whisper-cli -m " & quoted form of whisperModel & " -f " & quoted form of tempWav & " --no-timestamps -l zh 2>/dev/null"
